...
public abstract class PaletteBlockPartial<B extends Block> {
	...
	public @NonnullType BlockBuilder<B, CreateRegistrate> create_re(String variantName, PaletteBlockPattern pattern,
		BlockEntry<? extends Block> block, AllPaletteStoneTypes variant) {
		String patternName = Lang.nonPluralId(pattern.create_reName(variantName));
		String blockName = patternName + "_" + this.name;

		BlockBuilder<B, CreateRegistrate> blockBuilder = Create.REGISTRATE
			.block(blockName, p -> create_reBlock(block))
			.blockstate((c, p) -> generateBlockState(c, p, variantName, pattern, block))
			.recipe((c, p) -> create_reRecipes(variant, block, c, p))
	...
	protected abstract B create_reBlock(Supplier<? extends Block> block);

	protected abstract void create_reRecipes(AllPaletteStoneTypes type, BlockEntry<? extends Block> patternBlock,
	...
	private static class Stairs extends PaletteBlockPartial<StairBlock> {
		...
		@Override
		protected StairBlock create_reBlock(Supplier<? extends Block> block) {
		...
		@Override
		protected void create_reRecipes(AllPaletteStoneTypes type, BlockEntry<? extends Block> patternBlock,
			DataGenContext<Block, ? extends Block> c, RegistrateRecipeProvider p) {
	...
	private static class Slab extends PaletteBlockPartial<SlabBlock> {
		...
		@Override
		protected SlabBlock create_reBlock(Supplier<? extends Block> block) {
		...
		protected void generateBlockState(DataGenContext<Block, SlabBlock> ctx, RegistrateBlockstateProvider prov,
			...
			if (customSide) {
				...
				doubleSlab = prov.models()
					.getExistingFile(prov.modLoc(pattern.create_reName(variantName)));
			}
		...
		protected BlockBuilder<SlabBlock, CreateRegistrate> transformBlock(
			...
			builder.loot((lt, block) -> lt.add(block, lt.create_reSlabItemTable(block)));
	...
	private static class Wall extends PaletteBlockPartial<WallBlock> {
		...
		@Override
		protected WallBlock create_reBlock(Supplier<? extends Block> block) {
		...
		protected void generateBlockState(DataGenContext<Block, WallBlock> ctx, RegistrateBlockstateProvider prov,
			String variantName, PaletteBlockPattern pattern, Supplier<? extends Block> block) {
			prov.wallBlock(ctx.get(), pattern.create_reName(variantName), getTexture(variantName, pattern, 0));
		}
